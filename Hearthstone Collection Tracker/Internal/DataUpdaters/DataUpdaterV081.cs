...
{
    class DataUpdaterV081 : IDataUpdater
    {
        private static readonly Version _version = new Version(0, 8, 1);
        ...
        public void PerformUpdate()
        {
            ...
            foreach (var file in Directory.GetFiles(HearthstoneCollectionTrackerPlugin.PluginDataDir, "Collection_*.xml", SearchOption.TopDirectoryOnly))
            {
                ...
                if (setInfos.Any(s => s.SetName == KaCSet))
                    continue;
                ...
                {
                    SetName = KaCSet,
                    Cards = cards.Where(c => c.Set == KaCSet).Select(c => new CardInCollection()
                    {
                        AmountGolden = 0,
                        AmountNonGolden = 0,
                        CardId = c.Id,
